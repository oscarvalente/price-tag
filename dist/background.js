(function () {
    ...
    ***************************************************************************** */
    ...
    function getStorageDomain(domain) {
      ...
      }).pipe(take(1), map(result => parseDomainState(result, domain)));
    }
    ...
    function listenNotificationsButtonClicked() {
      ...
      switchMap(({
        notificationId,
        buttonIndex
      }) => StateManager.getNotification$(notificationId).pipe(map(notification => ({
        notification,
        buttonIndex
      ...
      switchMap(({
        ...
        return getStorageDomain(domain).pipe(filter(domainState => domainState[url]), map(domainState => ({
          domainState,
          domain,
          url,
          type,
    ...
    function onClearedNotification(notificationId, wasClosedByUser) {
      return clearNotification(notificationId).pipe(map(wasClickedByUser => ({
        ...
        return getStorageDomain(domain).pipe(map(domainState => {
          ...
          const item = ItemFactory.createItemFromObject(domainState[url]);
          const updateItemFn = get_1(onUpdateItemMapping$1, [type]);
    ...
    function onXHR$(url) {
      return from(makeXHR(url)).pipe(take(1));
    }
    ...
    function updatePriceUpdateStatus(url, domain, fullURL) {
      return getStorageDomain(domain).pipe(map(domainState => {
        const item = domainState[url] && ItemFactory.createItemFromObject(domainState[url]) || domainState[fullURL] && ItemFactory.createItemFromObject(domainState[fullURL]);
    ...
    function updateExtensionAppearance(currentDomain, currentURL, forcePageTrackingTo, fullURL) {
      if (forcePageTrackingTo === true) {
        ...
        return getStorageDomain(currentDomain).pipe(map(domainState => {
    ...
    function updateStatusAndAppearance(currentURL, domain, forcePageTrackingTo, url) {
      return forkJoin(updateAutoSaveStatus(currentURL, domain, url), updatePriceUpdateStatus(currentURL, domain, url), updateExtensionAppearance(domain, currentURL, forcePageTrackingTo, url));
    }
      ...
      if (domain) {
        ...
        return getStorageDomain(domain).pipe(switchMap(domainState => {
          ...
          if (domainState && domainState._canUseCanonical === false) {
            ...
            StateManager.updateBrowserURL(url);
            return onXHR$(url).pipe(switchMap(template => {
                      ...
                      case 1:
                        switch (index) {
                          case 0:
                            ...
                            return message$.pipe(mapTo([currentURL, true]));

                          case 1:
                            return message$.pipe(mapTo([currentURL, false, true]));

                          case 2:
                            // said No: same item (path is enough for this site items)
                            domainState._isPathEnoughToTrack = true;
                            return forkJoin(message$, setStorageDomain(domain, domainState)).pipe(mapTo([currentURL, false, false]));
    ...
    const DISCONNECTED_PORT_MSG = "Attempting to use a disconnected port object";
    ...
    function getTrackedItemsSortedBy(sortType) {
      return getStorage().pipe(map(result => {
        let trackedItems = [];
    ...
    function removeTrackedItem(url, currentURL, fullURL) {
      return getStorage().pipe(switchMap(result => {
          ...
          if (matchesDomain(domain)) {
            ...
            if (item) {
              ...
              return sendRuntimeMessage({
                ...
                return setStorageDomain(domain, domainItems).pipe(switchMap(() => currentURL === url || fullURL === url ? updateStatusAndAppearance(url, domain, false, fullURL) : of(undefined)));
              }));
            }
          }
        }
        ...
        for (let i = 0; i < resultKeys.length; i++) {
          ...
          if (matchesDomain(domain)) {
            ...
            if (item) {
              ...
              domainItems[url] = item;
              return setStorageDomain(domain, domainItems).pipe(switchMap(() => currentURL === url || fullURL === url ? updateStatusAndAppearance(url, domain, true, fullURL) : of(undefined)));
            }
          }
        }
      ...
      if (!isEmpty_1(statusUpdate)) {
        ...
        if (autoSaveUpdateStatus) {
          const {
            ...
            selection
          } = autoSaveUpdateStatus;
          enableAutoSave ? StateManager.enableAutoSave(selection) : StateManager.disableAutoSave();
        }

        if (priceUpdateStatus) {
          const {
            enable: enablePriceUpdate,
            selection
          } = priceUpdateStatus;
          enablePriceUpdate ? StateManager.enablePriceUpdate(selection) : StateManager.disablePriceUpdate();
        }
    ...
    function onRecordDone$(tabId, url, domain, payload) {
      ...
      })) : checkURLSimilarity(tabId, domain, url)), filter(([, isToSave]) => isToSave), switchMap(([url]) => forkJoin(createItem(domain, url, selection, price, State.faviconURL, faviconAlt, [ITEM_STATUS.WATCHED]), updateExtensionAppearance(domain, url, true))));
    }
    ...
    function onAutoSaveCheckStatus({
      status,
      selection,
      price,
      faviconURL,
    ...
    function listenRecordAttempt() {
      return onMessage(RECORD_ATTEMPT, ({
        payload: data,
        sendResponse$
      }) => {
        const {
        ...
        if (isRecordActive) {
          ...
          message$ = sendTabMessage(id, {
            ...
            status
          }) => status > 0), switchMap(onRecordDone$.bind(null, id, url, domain)), tap(([, isItemTracked]) => {
            updateExtensionAppearance$1(isItemTracked);
          }));
        } else {
          message$ = sendTabMessage(id, {
            type: RECORD_CANCEL
    ...
    function listenAutoSaveStatus() {
      return onMessage(AUTO_SAVE_STATUS, ({
        payload: data,
        sendResponse$
      }) => {
        const {
    ...
    function catchDisconnectedPort() {
      return catchError(({
        message
      }) => message === DISCONNECTED_PORT_MSG ? of(false) : EMPTY);
    }

    function listenAutoSaveAttempt() {
      return onMessage(AUTO_SAVE_ATTEMPT, ({
        payload: data,
        sendResponse$
      }) => {
        const {
        ...
        if (autoSaveEnabled) {
          ...
          return canDisplayURLConfirmation(state, domain).pipe(switchMap(canDisplay => {
            if (canDisplay) {
              return onCreateItemConfirm(id, domain, currentURL, selection, price, faviconURL, faviconAlt).pipe(filter(([canSave]) => canSave), switchMap(([, useCaninocal]) => {
                ...
                return checkURLSimilarity(id, domain, url).pipe(switchMap(([url, isToSave, autoSaveStatus]) => {
                  if (isToSave) {
                    ...
                    }), updateExtensionAppearance(domain, url, true))), tap(([highlightStopPayload, isItemTracked]) => {
                      onSimilarElementHighlight(highlightStopPayload);
                      updateExtensionAppearance$1(isItemTracked);
                    }), switchMap(() => sendResponse$(false)), catchDisconnectedPort());
                  } else {
                    ...
                    return sendResponse$(false).pipe(catchDisconnectedPort());
                  }
                }));
              }));
            } else {
              ...
              }), updateExtensionAppearance(domain, currentURL, true))), tap(([highlightStopPayload, isItemTracked]) => {
                onSimilarElementHighlight(highlightStopPayload);
                updateExtensionAppearance$1(isItemTracked);
              }), switchMap(() => sendResponse$(false)), catchDisconnectedPort())));
            }
    ...
    function listenAutoSaveHighlightPreStart() {
      return onMessage(AUTO_SAVE_HIGHLIGHT_PRE_START, ({
        payload: data
      }) => {
        const {
          payload = {}
    ...
    function listenAutoSaveHighlightPreStop() {
      return onMessage(AUTO_SAVE_HIGHLIGHT_PRE_STOP, ({
        payload: data
      }) => {
        const {
          payload = {}
    ...
    function listenPriceUpdateHighlightPreStart() {
      return onMessage(PRICE_UPDATE_HIGHLIGHT_PRE_START, ({
        payload: data
      }) => {
        const {
          payload = {}
    ...
    function listenPriceUpdateHighlightPreStop() {
      return onMessage(PRICE_UPDATE_HIGHLIGHT_PRE_STOP, ({
        payload: data
      }) => {
        const {
          payload = {}
    ...
    function listenTrackedItemsOpen(sortItemsByType) {
      return onMessage(TRACKED_ITEMS_OPEN, ({
        sendResponse$
      }) => {
    ...
    function listenTrackedItemsGet() {
      return onMessage(TRACKED_ITEMS_GET, ({
        sendResponse$
      }) => {
        const {
          _sortItemsBy
        } = StateManager.getState();
        return getTrackedItemsSortedBy(_sortItemsBy).pipe(switchMap(sortedItemsList => sendResponse$(sortedItemsList)));
      });
    }

    function listenTrackedItemsUnfollow() {
      return onMessage(TRACKED_ITEMS_UNFOLLOW, ({
        payload: data,
        sendResponse$
      }) => {
        const {
        ...
        return removeTrackedItem(url, currentURL, browserURL).pipe(tap(onStatusAndAppearanceUpdate), switchMap(sortedItemsList => sendResponse$(sortedItemsList)));
      });
    }

    function listenTrackedItemsChangeSort() {
      return onMessage(TRACKED_ITEMS_CHANGE_SORT, handler$ => of(handler$)).pipe(tap(({
        payload: data
      }) => {
        const {
          payload = {}
    ...
    function listenTrackedItemsUndoAttempt() {
      return onMessage(TRACKED_ITEMS_UNDO_ATTEMPT, ({
        sendResponse$
      }) => {
        const {
          _undoRemovedItems
        ...
        if (_undoRemovedItems.length > 0) {
          ...
          return undoRemoveTrackedItem(undoRemovedItem.url, currentURL, browserURL).pipe(tap(onStatusAndAppearanceUpdate), switchMap(() => {
            const {
              _undoRemovedItems
            } = StateManager.removeUndoRemovedItem(State);
            return _undoRemovedItems.length === 0 ? sendResponse$({
              isUndoStatusActive: false
            }) : sendResponse$({
              isUndoStatusActive: true
            });
        ...
        if (textContent) {
          ...
          if (textContentMatch) {
            ...
            if (!targetPrice) {
              ...
              domainItems[url] = item;
              return setStorageDomain(domain, domainItems).pipe(switchMap(() => {
                const {
                  notificationsCounter
                } = StateManager.getState();
                const notificationId = `TRACK.PRICE_FIXED-${notificationsCounter}`;
                return createCustomNotification(notificationId, PRICE_FIX_ICON, "Fixed price", `Ermm.. We've just fixed a wrongly set price to ${newPrice}`, url, url, domain);
              }));
              ...
              return setStorageDomain(domain, domainItems).pipe(switchMap(() => {
                ...
                if (newPrice < targetPrice && !item.hasAcknowledgeDecrease()) {
                  ...
                  return createCustomNotification(notificationId, PRICE_UPDATE_ICON, "Lower price!!", `${newPrice} (previous ${targetPrice})`, url, url, domain, ITEM_STATUS.DECREASED, {
                    buttons: [{
                      title: "Stop watching"
                    }, {
              ...
              return setStorageDomain(domain, domainItems).pipe(switchMap(() => {
                ...
                if (!item.hasAcknowledgeIncrease()) {
                  ...
                  return createCustomNotification(notificationId, PRICE_UPDATE_ICON, "Price increase", `${newPrice} (previous ${domainItems[url].previousPrice})`, url, url, domain, ITEM_STATUS.INCREASED, notificationOptions);
                }
              ...
              domainItems[url] = item;
              return setStorageDomain(domain, domainItems); // TODO: sendResponse("done"); // foi actualizado ou não
            } else {
              item.updateTrackStatus(null, null, [ITEM_STATUS.NOT_FOUND]);
              domainItems[url] = item;
        ...
        if (item.price && !newPrice) {
          if (!item.isNotFound()) {
            ...
            return setStorageDomain(domain, domainItems).pipe(switchMap(() => {
              ...
              return createCustomNotification(notificationId, PRICE_NOT_FOUND_ICON, "Price gone", `Price tag no longer found${previousPrice}`, url, url, domain);
            }));
          }
        }
      } catch (e) {
    ...
    function checkPriceChanges(freq) {
      return interval(freq).pipe(startWith(0), switchMap(() => getStorage().pipe(filter(result => result), mergeMap(result => {
        const domainsList = Object.keys(result);
        return range(0, domainsList.length).pipe(map(i => domainsList[i]), filter(domain => result.hasOwnProperty(domain) && matchesDomain(domain)), map(domain => ({
          domain,
          result
        })));
      }), mergeMap(({
        ...
        return range(0, urlsList.length).pipe(map(j => urlsList[j]), filter(url => domainItems.hasOwnProperty(url) && matchesURL(url)), map(url => ({
          url,
          domainItems,
          domain
        })));
        ...
        if (item.isWatched()) {
          ...
          return onXHR$(url).pipe(mergeMap(template => onPageFetch(template, domain, url, domainItems, item, targetPrice, currentPrice)));
        } else {
          return EMPTY;
        }
      }))));
    ...
    function attachEvents() {
      ...
      listenCompletedTab().pipe(switchMap(({
        id,
        url
      ...
      listenTrackedItemsOpen(TIME).subscribe();
      ...
      listenNotificationsButtonClicked().pipe(filter(hasStoppedWatch => hasStoppedWatch), switchMap(() => {
